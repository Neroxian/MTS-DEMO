<template>
  <div class=" card-box">
    <h3 class="bg-light p-2 text-uppercase">Edit Vendor</h3>

    <!-- Parent Form -->
    <b-form @submit.prevent="edit">
      <div class="text-center form-step">
        <span v-for="(step, idx) in steps" :key="idx">
          <span role="button" @click="currentStep = idx" :class="{ 'text-primary': idx === currentStep }">{{ step }}</span>
      ...
      <div :class="{ hidden: currentStep !== 0 }">
        ...
        <b-form-group label="Company Type: " label-for="company-type">
          <b-form-select v-model="data.companyType" :options="companyTypeOptions" />
        </b-form-group>
      ...
      <div :class="{ hidden: currentStep !== 2 }">
        ...
        <div class="row">
          <b-form-group
            ...
            label-for="state"
          >
            <b-form-select id="state" v-model="data.state" :options="stateOptions" class="shadow-none" />
          </b-form-group>
          <b-form-group
            class="col-12 col-md-6"
            label="District: "
